...
if (isset($_POST["Email"])) {
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'adisahrul383@gmail.com';  // Alamat email Anda
        $mail->Password   = 'yuds tter bmcy djus';           // Password email Anda
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS; // Enable TLS encryption; `PHPMailer::ENCRYPTION_STARTTLS` also accepted
        $mail->Port       = 465;                        // Port TCP untuk menghubungkan ke server
    
        // Penerima
        $mail->setFrom('MonkaselSurabaya@gmail.com','Monumen Kapal Selam');
        $mail->addAddress($emailTo); // Tambahkan penerima
        $mail->addReplyTo('MonKaselSurabaya@gmail.com');
    
        // Konten email
        $url = "http://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["PHP_SELF"]) . "/resetpassword.php?code=$code";
        $mail->isHTML(true);
        $mail->Subject = 'Monumen Kapal Selam';
        $mail->Body    = "<h1> Permintaan Reset Password Anda
                            Klik <a href='$url'>Disini</a> Untuk Reset Password";
        $mail->AltBody = 'Ini adalah pesan dalam format teks biasa untuk klien email yang tidak mendukung HTML';
    
        $mail->send();
        echo '<script>alert("Reset password link Sudah Terkirim di Email Anda!");</script>';
    } catch (Exception $e) {
...
<div class="content"> 
    <div class="content-box">
        <h1 style="margin-bottom: 0;">Lupa Password</h1>
        <hr style="width: 100%; margin-bottom: 20px; border: 3px solid #2697FF;">
        <form method="post">
            <label for="Email">Email</label>
            <input type="email" name="Email" required>
            <div class="container-btn">
                <button class="btnn" type="submit" name="submit">Minta Kode</button>
            </div>
        </form>
    </div>
